...
def index(request):
    ...
    all_deals = Product.objects.filter(product_status="published", deals=True).order_by("-id")
    top_selling = Product.objects.filter(product_status="published", selling=True).order_by("-id")
    trending_products = Product.objects.filter(product_status="published", trending=True).order_by("-id")
    recently_added = Product.objects.filter(product_status="published", recent=True).order_by("-id")
    top_rated = Product.objects.filter(product_status="published", top=True).order_by("-id")
...
def filter_product(request):
    ...
    if len(colors) > 0:
        ...
        products = Product.objects.filter(product_status="published").order_by("-id").distinct()
    if len(brands) > 0:
        products = products.filter(brands__id__in=brands).distinct() 
    else:
        products = Product.objects.filter(product_status="published").order_by("-id").distinct()
    if len(sizes) > 0:
...
def make_payment(request):
    ...
    if 'cart_data_obj' in request.session:
        for p_id, item in request.session['cart_data_obj'].items():
            cart_total_amount += int(item['qty']) * float(item['price'])
    return render(request, 'core/payment.html',  {'cart_data':request.session['cart_data_obj'],'totalcartitems':len(request.session['cart_data_obj']),'cart_total_amount':cart_total_amount})
